        ...
        <div key={step.id} className="flex items-center w-full">
          <div className="relative flex flex-col items-center flex-grow">
            {index < steps.length - 1 && (
              <div className={`absolute top-5 left-1/2 w-full h-1 -translate-y-1/2 ${currentStep > step.id ? 'bg-green-600' : 'bg-gray-300'}`}></div>
            )}
            ...
            <span className={`mt-2 text-xs text-center ${currentStep >= step.id ? 'text-green-600 font-semibold' : 'text-gray-400'}`}>
              {step.label}
            </span>
          </div>
        </div>
